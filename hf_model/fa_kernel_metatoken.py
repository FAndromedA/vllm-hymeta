...
class FlashAttentionWithMetaToken(torch.autograd.Function):
    ...
    def forward(
        ...
        # if q2 is not None:
        #     print(q1.dtype, q2.dtype)
        if q1.dtype != torch.bfloat16:
            q1 = q1.to(torch.bfloat16)
            k1 = k1.to(torch.bfloat16)
            v1 = v1.to(torch.bfloat16)
            if q2 is not None:
                q2 = q2.to(torch.bfloat16)
            k2 = k2.to(torch.bfloat16)
        ...
        out1 = _flash_attn_forward(
            ...
            causal=True,
            window_size_left=window_size[0]-1,
            window_size_right=-1,
        ...
        ctx.num_meta_tokens = num_meta_tokens
    ...
    def backward(ctx, dout):
        ...
        num_meta_tokens = ctx.num_meta_tokens
        Lq = q1.shape[1]
        ...
        with torch.no_grad():
            ...
            dq1 = dq1 + dq2[:,num_meta_tokens:]
            dq2 = dq2[:,:num_meta_tokens] if q2 is not None else None

        return dq1, dq2, dk1, dk2, dv1, dv2, None, None, None, None, None, None, None, None, None
...
class FlashAttentionVarlenWithMetaToken(torch.autograd.Function):
    ...
    def forward( 
        ...
        out1, lse1 = out1[0].unsqueeze(0), out1[5].unsqueeze(0)
        # einops.EinopsError:  Error while processing rearrange-reduction pattern "b h l -> 1 h (b l)".
        # Input tensor shape: torch.Size([28, 424]). Additional info: {}.
