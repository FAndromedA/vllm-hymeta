...
class Cache(transformers.cache_utils.Cache):
    ...
    def update(
        ...
        meta_state: Tuple[torch.Tensor, torch.Tensor] = None,
        ...
        Args:
            ...
            meta_state (`Tuple[torch.Tensor, torch.Tensor]`, `optional`):
                The new meta tokens key/value states to cache.
        ...
        if meta_state is not None:
            if not isinstance(meta_state, Tuple) or len(meta_state) != 2:
                raise ValueError("`meta_state` must be a tuple of two tensors for key/value states")
